...
for($i = 2; $i <= $highestRow; $i++) {
    ...
    $func_name =        $worksheet->getCell('B'.$i)->getValue();
    $prod_name =    $worksheet->getCell('C'.$i)->getValue();
    $prod_main_img = $worksheet->getCell('D'.$i)->getValue();
    $prod_size =       $worksheet->getCell('E'.$i)->getValue();
    $prod_price =     $worksheet->getCell('F'.$i)->getValue();
    $rental_charge =          $worksheet->getCell('G'.$i)->getValue();
    $rental_day =          $worksheet->getCell('H'.$i)->getValue();
    $prod_description =          $worksheet->getCell('I'.$i)->getValue();
    $feature_id =          $worksheet->getCell('J'.$i)->getValue();
    $cat_id =          $worksheet->getCell('K'.$i)->getValue();
    
    //寫入資料
    $sql = "INSERT INTO `product`(
        `id`, `func_name`, `prod_name`, `prod_main_img`, `prod_size`, 
        `prod_price`, `rental_charge`, `rental_day`, `prod_description`, `feature_id`, `cat_id`
        ) VALUES (
            {$id}, '{$func_name}', '{$prod_name}', '{$prod_main_img}', '{$prod_size}', 
            '{$prod_price}', {$rental_charge}, {$rental_day}, {$prod_description}, {$feature_id}, {$cat_id}
            )";
