...
if( isset($_POST['ach_id']) && isset($_POST['name']) && isset($_POST['email']) && isset($_POST['address']) && isset($_POST['phone']) ){
    try{
        $sql = "SELECT * FROM `reward`
                WHERE `ach_id` =  '{$_POST['ach_id']}'";
        ...
        if($stmt1->rowCount() > 0){
            ...
            $sql3 = "INSERT INTO `orders_detail` (`order_id`, `prod_id`, `prod_name`, `prod_main_img`, `func_name`, `prod_description`, `prod_price`, `prod_qty`, `subtotal`) VALUES ('{$order_id}', '{$_POST['ach_id']}', '{$arr['prod_name']}', '{$arr['prod_main_img']}', '21', '{$arr['prod_description']}', 0, 1, 0)";
            $pdo->query($sql3);


            $sql5 = "UPDATE `users` SET `{$_POST['ach_id']}` = 'completed recieved' WHERE `email` = '{$_SESSION['email']}'";
